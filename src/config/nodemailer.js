...
const signupMailUni = (data, host, protocol) => {
    const maxAge = 3 * 60 * 60
    ...
    const PORT = process.env.PORT || 3000
    const link = `${protocol}://${host}:${PORT}/university/verify/${data._id}?tkn=${TOKEN}`
...
const passwordMailUni = (user, TOKEN, host, protocol) => {
    const PORT = process.env.PORT || 3000
    const link = `${protocol}://${host}:${PORT}/university/resetPassword/${user._id}/${TOKEN}`
